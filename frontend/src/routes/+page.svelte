<script lang="ts">
  ...
  function toggleGame(gamePk: number) {
    ...
    selectedGamePks = new SvelteSet(selectedGamePks);
  }
...
<main class="max-w-3xl mx-auto p-6 space-y-8 text-gray-900">
  <h1 class="text-3xl font-bold">Pitcher Stats</h1>

  <ErrorMessage message={error} />

  <section class="flex gap-2 items-center flex-wrap">
    <TextInput bind:value={name} placeholder="Pitcher Name" />
    <Button onclick={doSearch}>Suchen</Button>

    {#if matches.length > 0}
      <select bind:value={selectedPitcherId} class="border border-gray-300 rounded px-3 py-2">
  ...
  <section class="flex gap-2 items-center flex-wrap">
    ...
    <Button onclick={loadGames} disabled={loading}>Spiele laden</Button>
  </section>
